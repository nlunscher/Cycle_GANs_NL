...
class cycle_IN():
    ...
    def __init__(self, sess):
        ...
        self.data_category = 'Horse_train'
        ...
        self.main_folder = 'cin_saves_avgpool_mask/'
    ...
    def create_net(self):
        ...
        a2b_W_dconv2 = weight_variable([7, 7, 3+1, 32])
        a2b_b_dconv2 = bias_variable([3+1])
        ...
        def a2b(a_image, is_train):
            ...
            out_pix = tf.slice(h_dconv2, [0,0,0,0], [self.batch_size, self.im_size, self.im_size, 3])
            mask = tf.slice(h_dconv2, [0,0,0,3], [self.batch_size, self.im_size, self.im_size, 1])

            out = a_image * (1-mask) + out_pix * mask
        ...
        b2a_W_dconv2 = weight_variable([7, 7, 3+1, 32])
        b2a_b_dconv2 = bias_variable([3+1])
        ...
        def b2a(b_image, is_train):
            ...
            out = b_image * (1-mask) + out_pix * mask
        ...
        # G
        self.a2b_im, self.a2b_mask = a2b(self.a_im, self.is_train)
        self.b2a_im, self.b2a_mask = b2a(self.b_im, self.is_train)

        self.a2b2a_im, self.a2b2a_mask = b2a(self.a2b_im, self.is_train)
        self.b2a2b_im, self.b2a2b_mask = a2b(self.b2a_im, self.is_train)
        ...
        self.cycle_loss = 2.5 * (tf.reduce_mean(tf.reduce_sum(tf.abs(self.a2b2a_im - self.a_im), 3)) +
                                tf.reduce_mean(tf.reduce_sum(tf.abs(self.b2a2b_im - self.b_im), 3)))

        ### Mask
        mask_fraction = 0.05
        self.a2b_m_loss = tf.abs(tf.minimum(tf.reduce_sum(self.a2b_mask) / (self.im_size**2), mask_fraction) - mask_fraction) / mask_fraction
        self.b2a_m_loss = tf.abs(tf.minimum(tf.reduce_sum(self.b2a_mask) / (self.im_size**2), mask_fraction) - mask_fraction) / mask_fraction

        self.a2b_m_v_loss = tf.reduce_mean(tf.abs(tf.round(self.a2b_mask) - self.a2b_mask)) * 0.5
        self.b2a_m_v_loss = tf.reduce_mean(tf.abs(tf.round(self.b2a_mask) - self.b2a_mask)) * 0.5

        # self.a2b_m_loss = tf.abs(tf.minimum(tf.reduce_sum(tf.round(self.a2b_mask)) / (self.im_size**2), mask_fraction) - mask_fraction) * 5
        # self.b2a_m_loss = tf.abs(tf.minimum(tf.reduce_sum(tf.round(self.b2a_mask)) / (self.im_size**2), mask_fraction) - mask_fraction) * 5


        self.mask_loss = self.a2b_m_loss + self.b2a_m_loss + self.a2b_m_v_loss + self.b2a_m_v_loss
        self.G_loss = self.a2b_Ad + self.b2a_Ad + self.cycle_loss + self.mask_loss

        # D
        self.b_D_fake, self.b_D_fake_sig = self.b_G_fake, self.b_G_fake_sig
        self.b_D_real, self.b_D_real_sig = bD(self.b_im, self.is_train)

        self.a_D_fake, self.a_D_fake_sig = self.a_G_fake, self.a_G_fake_sig
    ...
    def train(self, iterations):
        ...
        cv2.resizeWindow("Images", 256*5, 256*2)
        ...
        for i in range(iterations):
            ...
            _, _, m, a2b, a2b_m, a2b2a, a2b2a_m, s, b2a, b2a_m, b2a2b, b2a2b_m, \
                        a2bDS, b2aDS, \
                        a2bD, b2aD, cL, mL, \
                        aDS, aDFS, bDS, bDFS, \
                        aD, mFD, bD, sFD = self.sess.run([G_train_step, D_train_step, 
                                                self.a_im, self.a2b_im, self.a2b_mask, self.a2b2a_im, self.a2b2a_mask, self.b_im, self.b2a_im, self.b2a_mask, self.b2a2b_im, self.b2a2b_mask,
                                                self.b_G_fake_sig, self.a_G_fake_sig, 
                                                self.a2b_Ad, self.b2a_Ad, self.cycle_loss, self.mask_loss,
            ...
            if (i % 100 == 0 or i == iterations-1):
                ...
                print "        ", "a2bD:", a2bD, "b2aD:", b2aD, "cL:", cL, "mL:", mL
                print "        ", "aDS:", aDS[0], "aDFS:", aDFS[0], "bDS:", bDS[0], "bDFS:", bDFS[0]
                print "        ", "aD:", aD, "mFD:", mFD, "bD:", bD, "sFD:", sFD

                a2b_m = np.dstack((a2b_m[0],a2b_m[0],a2b_m[0]))
                b2a_m = np.dstack((b2a_m[0],b2a_m[0],b2a_m[0]))
                a2b2a_m = np.dstack((a2b2a_m[0],a2b2a_m[0],a2b2a_m[0]))
                b2a2b_m = np.dstack((b2a2b_m[0],b2a2b_m[0],b2a2b_m[0]))

                x = np.concatenate([np.concatenate([m[0], a2b[0], a2b_m, a2b2a[0], a2b2a_m], axis = 1),
                                    np.concatenate([s[0], b2a[0], b2a_m, b2a2b[0], b2a2b_m], axis = 1)], axis = 0)
                cv2.imshow('Images', x)
                cv2.waitKey(50)
