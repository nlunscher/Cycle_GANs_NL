...
class cycle_IN():
    ...
    def __init__(self, sess):
        ...
        self.main_folder = 'cin_saves64/'
        make_directory(self.main_folder)
        self.save_folder = self.main_folder + '64_DOG'
    ...
    def create_net(self):
        ...
        self.bD_vars = [bD_W_conv1, bD_W_conv2, bD_W_conv3, bD_W_conv4, bD_W_f1, bD_b_f1, bD_W_f2, bD_b_f2]
        ...
        def bD(b_image, is_train):
            ...
            h_conv2 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv1, bD_W_conv2, 2), training=is_train), name="bD_conv2") #16
            h_conv3 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv2, bD_W_conv3, 2), training=is_train), name="bD_conv3") #8
            h_conv4 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv3, bD_W_conv4, 2), training=is_train), name="bD_conv4") #4
            h_flat = tf.reshape(h_conv4, [-1, 4*4*64])
            h4 = tf.nn.sigmoid(tf.matmul(h_flat, bD_W_f1) + bD_b_f1, name="bD_f1")
        ...
        self.aD_vars = [aD_W_conv1, aD_W_conv2, aD_W_conv3, aD_W_conv4, aD_W_f1, aD_b_f1, aD_W_f2, aD_b_f2]
        ...
        def aD(a_image, is_train):
            ...
            h_conv2 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv1, aD_W_conv2, 2), training=is_train), name="aD_conv2") #16
            h_conv3 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv2, aD_W_conv3, 2), training=is_train), name="aD_conv3") #8
            h_conv4 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv3, aD_W_conv4, 2), training=is_train), name="aD_conv4") #4
            h_flat = tf.reshape(h_conv4, [-1, 4*4*64])
            h4 = tf.nn.sigmoid(tf.matmul(h_flat, aD_W_f1) + aD_b_f1, name="aD_f1")
        ...
        self.cycle_loss = 2 * (tf.reduce_mean(tf.reduce_sum(tf.abs(self.a2b2a_im - self.a_im), 3)) +
