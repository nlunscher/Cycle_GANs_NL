...
class cycle_IN():
    ...
    def __init__(self, sess):
        ...
        self.data_folder = 'Data_ImageNet/fast_data/Horse_train/'
        pair = ['trainA', 'trainB']
        ...
        self.main_folder = 'cin_saves64_avgpool/'
        make_directory(self.main_folder)
        self.save_folder = self.main_folder + '64_Horse'
    ...
    def load_pair(self):
        ...
        a = cv2.resize(a, (64,64), interpolation = cv2.INTER_LINEAR)
    ...
    def create_net(self):
        self.a_im = tf.placeholder(tf.float32, name='a_image',shape=[self.batch_size] + [64,64,3])
        self.b_im = tf.placeholder(tf.float32, name='b_image',shape= [self.batch_size] + [64,64,3])
        ...
        def a2b(a_image, is_train):
            ...
            h_dconv1 = tf.nn.relu(conv2d_transpose(h_c6_2, a2b_W_dconv1, osize_dconv1,2) + a2b_b_dconv1, name="a2b_dconv1") #32
        ...
        bD_W_conv1 = weight_variable([5, 5, 3, 32])
        bD_W_conv2 = weight_variable([3, 3, 32, 64])
        bD_W_conv3 = weight_variable([3, 3, 64, 64])
        bD_W_conv4 = weight_variable([3, 3, 64, 64])
        bD_b_conv4 = bias_variable([64])
        bD_W_conv5 = weight_variable([1, 1, 64, 1])
        bD_b_conv5 = bias_variable([1])

        self.bD_vars = [bD_W_conv1, bD_W_conv2, bD_W_conv3, bD_W_conv4, bD_b_conv4, bD_W_conv5, bD_b_conv5]

        def bD(b_image, is_train):
            h_conv1 = tf.nn.relu(tf.layers.batch_normalization(conv2d(b_image, bD_W_conv1, 2), training=is_train), name="bD_conv1") #32
            h_conv2 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv1, bD_W_conv2, 1), training=is_train), name="bD_conv2") #32
            h_conv3 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv2, bD_W_conv3, 1), training=is_train), name="bD_conv3") #32
            h_conv4 = tf.nn.relu(conv2d(h_conv3, bD_W_conv4, 1) + bD_b_conv4, name="bD_conv4") #32
            h_conv5 = (conv2d(h_conv4, bD_W_conv5, 1) + bD_b_conv5) #32
            h_pool = tf.reshape(tf.nn.pool(h_conv5, [32,32], "AVG", "VALID"), [-1,1])
        ...
        def b2a(b_image, is_train):
            ...
            osize_dconv1 = h_conv1.get_shape().as_list()
            osize_dconv1[0] = self.batch_size
            h_dconv1 = tf.nn.relu(conv2d_transpose(h_c6_2, b2a_W_dconv1, osize_dconv1,2) + b2a_b_dconv1, name="b2a_dconv1") #32
        ...
        aD_W_conv1 = weight_variable([5, 5, 3, 32])
        aD_W_conv2 = weight_variable([3, 3, 32, 64])
        aD_W_conv3 = weight_variable([3, 3, 64, 64])
        aD_W_conv4 = weight_variable([3, 3, 64, 64])
        aD_b_conv4 = bias_variable([64])
        aD_W_conv5 = weight_variable([1, 1, 64, 1])
        aD_b_conv5 = bias_variable([1])

        self.aD_vars = [aD_W_conv1, aD_W_conv2, aD_W_conv3, aD_W_conv4, aD_b_conv4, aD_W_conv5, aD_b_conv5]

        def aD(a_image, is_train):
            h_conv1 = tf.nn.relu(tf.layers.batch_normalization(conv2d(a_image, aD_W_conv1, 2), training=is_train), name="aD_conv1") #32
            h_conv2 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv1, aD_W_conv2, 1), training=is_train), name="aD_conv2") #32
            h_conv3 = tf.nn.relu(tf.layers.batch_normalization(conv2d(h_conv2, aD_W_conv3, 1), training=is_train), name="aD_conv3") #32
            h_conv4 = tf.nn.relu(conv2d(h_conv3, aD_W_conv4, 1) + aD_b_conv4, name="aD_conv4") #32
            h_conv5 = tf.nn.relu(conv2d(h_conv4, aD_W_conv5, 1) + aD_b_conv5, name="aD_conv5") #32
            h_pool = tf.reshape(tf.nn.pool(h_conv5, [32,32], "AVG", "VALID"), [-1,1])
            h_sig = tf.nn.sigmoid(h_pool, name = "sD_sig")
